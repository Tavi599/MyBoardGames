...
# «бгг.py» ходить у мережу, але переклад чужого JSON у наші поля — річ
...
def бгг_тести():
    ...
    так("слуг із адреси", бгг.слуг("/boardgamecategory/1002/card-game"), "card-game")
    так("порожня адреса", бгг.слуг(None), "")
    так("нуль — це «невідомо»", бгг.ціле("0"), None)
    так("число рядком", бгг.ціле("45"), 45)
    так("не число", бгг.ціле("кілька"), None)
    так("діапазони розгортаються", бгг.склади([{"min": 3, "max": 4}]), [3, 4])
    так("склади злипаються без повторів",
        бгг.склади([{"min": 1, "max": 2}, {"min": 2, "max": 3}]), [1, 2, 3])
    так("стеля опитування", бгг.склади([{"min": 9, "max": 40}]), [9, 10])
    так("середня — з одним знаком", бгг.десятина("7.5754"), 7.6)

    # Той самий шлях, що й у мережі, лише відповіді підкладені.
    справжнє = бгг.взяти
    бгг.взяти = lambda url: ВІДПОВІДЬ_РЕЧІ if "geekitems" in url else ВІДПОВІДЬ_ЖИВОГО
    try:
        свіже = бгг.дістати("1")
    finally:
        бгг.взяти = справжнє
    ...
    порожньо = бгг.дістати.__doc__ is not None
    if not порожньо:
        зле.append("✗ бгг.дістати без опису")

    # Що і як лягає в запис.
    гра = {"bggMin": 2, "bggRating": 7.0, "cover": "https://своя/картинка.jpg",
           "minP": 3, "score": 9}
    змінені = бгг.застосувати(dict(гра), свіже, освіжати=False)
    ...
    бгг.застосувати(з_освіженням, свіже, освіжати=True)
    ...
    голяк = {}
    бгг.застосувати(голяк, свіже, освіжати=False)
    ...
    # на ньому справедливо спиняється «перевірка.py».
    вузька = {}
    бгг.застосувати(вузька, {"bggMax": 2, "bggBest": [1, 2], "bggRec": [1, 2, 3]},
                    освіжати=True)
    так("опитування не перестрибує стелю коробки", вузька.get("bggRec"), [1, 2])
    так("а те, що в межах, лишається", вузька.get("bggBest"), [1, 2])
    зовсім = {}
    бгг.застосувати(зовсім, {"bggMax": 2, "bggBest": [5, 6]}, освіжати=True)
...
def безпека_тести():
    ...
    for хост in ("localhost:8777", "127.0.0.1:8777", "192.168.1.14:8777",
        ...
        так(f"свій хост {хост}", сервер.свій_хост(хост), True)
    ...
    for хост in ("evil.example:8777", "полиця.zip", "localhost.evil.com",
        ...
        так(f"чужий хост {хост}", сервер.свій_хост(хост), False)

    так("Origin зі своєї ж адреси",
        сервер.своя_сторінка("http://192.168.1.14:8777", "192.168.1.14:8777"), True)
    так("Origin чужої сторінки",
        сервер.своя_сторінка("http://evil.example", "localhost:8777"), False)
    так("порожній Origin — це не браузерна сторінка",
        сервер.своя_сторінка(None, "localhost:8777"), True)
    # Найпідліший випадок: адреса, яка лише починається як наша.
    так("Origin, схожий на свій",
        сервер.своя_сторінка("http://localhost:8777.evil.com", "localhost:8777"), False)
