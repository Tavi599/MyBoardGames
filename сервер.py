...
def закомітити() -> None:
    ...
    with замок:
        стан = прочитати()
        помилки, _ = перевірка.перевірити(стан)
    ...
    if помилки:
        ...
        for р in помилки[:5]:
            print("  ✗", р)
        print('  Полагодь і запусти: python "перевірка.py"')
        return
    git("add", "--", ДАНІ.name)
...
class Обробник(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with замок:
            ...
            вади, _ = перевірка.перевірити(стан) if змінено else ([], [])
        if змінено:
    ...
    def _добрати_з_bgg(self, вхід: dict) -> None:
        ...
        піднімаємо — з тієї самої причини, що й у «бгг.py»: інакше цей запис
        ...
        with замок:
            ...
            гра = знайти(стан.get("games", []), ід)
            if гра is None:
                self._json({"error": "коробку прибрали, поки я питав"}, 404)
                return
            # Що у файлі було не так до нас — не наша провина й не привід
            # відмовлятися: інакше одна крива коробка зупинила б добирання
            # для всіх інших, ще й звинувативши в цьому BGG.
            # Копія, а не сам список: перевірка повертає свій власний, і
            # другий виклик просто перепише той самий об'єкт.
            було = list(перевірка.перевірити(стан)[0])
            поля = бгг.застосувати(гра, свіже, освіжати=True)
            if поля:
                стало = перевірка.перевірити(стан)[0]
                нові = [р for р in стало if р not in було]
                if нові:
                    # Нічого не записали: прочитане з диска лишилося на диску.
                    self._json({"error": "BGG віддав те, що не проходить "
                                         "перевірку: " + нові[0]}, 502)
                    return
                записати(стан)
            назовні = dict(гра)
        if поля:
