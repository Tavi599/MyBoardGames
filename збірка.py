...
# Файли коду без жодного звертання до сторінки — саме їх можна перевіряти
# окремо. Node у системі немає, тож тести живуть у власній сторінці.
ЧИСТА_ЛОГІКА = ["01-основа.js", "03-вибірка.js"]
...
def джерела() -> list[Path]:
    ...
    return свої + файли_коду() + [Path(__file__)]
...
def сторінка() -> str:
    ...
    код = "".join(читати(ф) for ф in файли_коду())
    return (голова + "\n<style>\n" + стилі + "</style>\n\n"
            + розмітка + "\n<script>\n" + код + "</script>\n")
...
def сторінка_тестів() -> str:
    код = "".join(читати(ДЖЕРЕЛА / "код" / і) for і in ЧИСТА_ЛОГІКА)
